...
class Trainer_AE_w_DDM:
    ...
    '''
    def train_PreT_2(self): # optimizing the encoder and the decoder iteratively
        encoder = self.encoder
        decoder = self.decoder 
        channel_gen = self.channel_gen 
        device = self.device 
        ...
        betas = tconf_AE.betas.to(device)
        alphas = tconf_AE.alphas.to(device)
        weight_decay = tconf_AE.weight_decay
        
        dataset_AE = np.random.randint(M, size=tconf_AE.dataset_size)
        loss_CE = nn.CrossEntropyLoss() 
        
        optimizer_dec = tconf_AE.optim_AE(decoder.parameters(), lr = tconf_AE.learning_rate, weight_decay = weight_decay)
        optimizer_enc = tconf_AE.optim_AE(encoder.parameters(), lr = tconf_AE.learning_rate, weight_decay = weight_decay)
        ...
        for t in range(tconf_AE.max_epochs):
            ...
            # Backward pass: 
                ...
                loss_batch_dec.append(loss.item())

                SER_tmp, _, _ = utils.SER(labels.long(), m_d, 0.9)
                batch_SER_dec.append(SER_tmp.item())

                if it%100==0:
                    pbar.set_description(f"<Decoder> epoch {t+1}: loss {np.mean(loss_batch_dec):.2e} SER {np.mean(batch_SER_dec):.2e}")        

            # Save epoch loss
            loss_ep_dec.append(np.mean(loss_batch_dec))
            print(sum(p.pow(2.0).sum() for p in decoder.parameters()).item())
            
            

            loader = DataLoader(dataset_AE, shuffle=True, pin_memory=False, batch_size=tconf_AE.batch_size)#, num_workers=config.num_workers)            
            loss_batch_enc, batch_SER_enc = [], []
            pbar = tqdm(enumerate(loader), total=len(loader), mininterval=0.5)
            ...
            # Backward pass: 
                ...
                loss_batch_enc.append(loss.item())

                SER_tmp, _, _ = utils.SER(labels.long(), m_d, 0.9)
                batch_SER_enc.append(SER_tmp.item())

                if it%100==0:
                    pbar.set_description(f"<Encoder> epoch {t+1}: loss {np.mean(loss_batch_enc):.2e} SER {np.mean(batch_SER_enc):.2e}")        
        
            # Save epoch loss
            loss_ep_enc.append(np.mean(loss_batch_enc))

        return loss_ep_dec #, loss_ep_enc
    '''
